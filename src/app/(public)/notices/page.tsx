...
import { NoticesFilterClient } from "./notices-filter-client"
...
export default async function PublicNoticesPage({ searchParams }: NoticesPageProps) {
  const { search = "", priority = "" } = await searchParams
  const now = new Date()
  ...
  const notices = await prisma.notice.findMany({
    ...
    select: {
      ...
      attachmentUrl: true, createdAt: true,
    },
  })
  ...
  return (
    <div className="relative min-h-screen bg-background overflow-hidden py-16 px-4 md:px-8">
      ...
      <div className="max-w-5xl mx-auto space-y-12 relative z-10">
        ...
        {/* Filter — client island (needs state for search/priority) */}
        <NoticesFilterClient initialSearch={search} initialPriority={priority} />
              ...
              >
                <div className="flex md:flex-col items-center gap-3">
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center border ${
                    notice.priority === "URGENT" ? "bg-red-500/10 text-red-500 border-red-500/20"
                    : notice.priority === "HIGH" ? "bg-amber-500/10 text-amber-500 border-amber-500/20"
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <p className="text-muted-foreground text-sm leading-relaxed whitespace-pre-line">{notice.description}</p>
